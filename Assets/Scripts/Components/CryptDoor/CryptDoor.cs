  ...
  private void OnTriggerEnter(Collider other)
  {
    if (other.CompareTag("Ball") && !isHoldingBall) // Ensure the collider is tagged as "Ball"
    {
      ...
      if (ballRigidbody != null)
      {
        ...
        if (unlocked)
        {
          gameManager.GetComponent<B2B3Transition>().StartTransition();
        } else if (SceneArguments.previousSceneCondition != "B2B3Loss") {
          Invoke(nameof(KickBall), holdTime); // Schedule the kick after the hold time
        }
      }
    }
  }
